...
def parse_axes(axes, default_bins, dist_fraction=1e-4/2):
    """
    Parse and discretize a list of axes

    :param axes: A list of axis definitions. Each axis is a `dict` with
    ...
    :param default_bins (int, optional): Number of bins for axes without explicit definition.
        Defaults to 100.
    :param dist_fraction  (float, optional): Fraction of the probability distribution
        discarded at each side of the sampling space. Defaults to 0.00005
...
def get_sobol_g(N, a=6.52, name_tmpl='x_{}'):
    ...
    def function(Xs, a=a):
        assert(Xs.shape[1] == N)
        return np.prod((np.abs(4*Xs-2) + a) / (1. + a), axis=1)
...
def get_piston():
    ...
    axes = [dict(name='M: piston weight (kg)', domain=(30, 60)),
            dict(name='S: piston surface area (m^{2})', domain=(0.005, 0.020)),
            dict(name='V_0: initial gas volume (m^{3})', domain=(0.002, 0.010)),
            dict(name='k: spring coefficient (N/m)', domain=(1000, 5000)),
            dict(name='P_0: atmospheric pressure (N/m^{2})', domain=(90000, 110000)),
            dict(name='T_a: ambient temperature (K)', domain=(290, 296)),
            dict(name='T_0: filling gas temperature (K)', domain=(340, 360))]
...
def get_fire_spread(wind_factor=1.0):
    ...
    axes = [dict(name='delta: Fuel depth', dist=sp.stats.lognorm(scale=np.exp(2.19), s=0.517), display_name=r"\delta"),
            dict(name='sigma: Fuel particle area-to-volume ratio', domain=(3.0 / 0.6, None), dist=sp.stats.lognorm(scale=np.exp(3.31), s=0.294), display_name=r"\sigma"),
            dict(name='h: Fuel particle low heat content', dist=sp.stats.lognorm(scale=np.exp(8.48), s=0.063)),
            dict(name='rho_p: Oven-dry particle density', dist=sp.stats.lognorm(scale=np.exp(-0.592), s=0.219), display_name=r"\rho_{p}"),
            dict(name='m_l: Moisture content of the live fuel', domain=(0, None), dist=sp.stats.norm(loc=1.18, scale=0.377)),
            dict(name='m_d: Moisture content of the dead fuel', dist=sp.stats.norm(loc=0.19, scale=0.047)),
            dict(name='S_T: Fuel particle total mineral content', domain=(0, None), dist=sp.stats.norm(loc=0.049, scale=0.011)),
            dict(name='U: Wind speed at midflame height', dist=sp.stats.lognorm(scale=np.exp(1.0174)*wind_factor, s=0.5569)),
            dict(name='tan_phi: Slope', domain=(0, None), dist=sp.stats.norm(loc=0.38, scale=0.186), display_name=r"tan\ \phi"),
            dict(name='P: Dead fuel loading to total fuel loading', domain=(None, 1), dist=sp.stats.lognorm(scale=np.exp(-2.19), s=0.64))]
...
def get_robot_arm():
    ...
    axes = [dict(name='\\phi_1', domain=(0, 2*np.pi)),
            dict(name='L_1', domain=(0, 1)),
            dict(name='\\phi_2', domain=(0, 2*np.pi)),
            dict(name='L_2', domain=(0, 1)),
            dict(name='\\phi_3', domain=(0, 2*np.pi)),
            dict(name='L_3', domain=(0, 1)),
            dict(name='\\phi_4', domain=(0, 2*np.pi)),
            dict(name='L_4', domain=(0, 1))]
...
def get_wing_weight():
    ...
    axes = [dict(name='Sw: wing area', domain=(150, 200)),
            dict(name='Wfw: weight of fuel in the wing', domain=(220, 300)),
            dict(name='A: aspect ratio', domain=(6, 10)),
            dict(name='Lambda: quarter-chord sweep (degrees)', domain=(-10, 10)),
            dict(name='q: dynamic pressure at cruise', domain=(16, 45)),
            dict(name='lambda: taper ratio', domain=(0.5, 1)),
            dict(name='tc: aerofoil thickness to chord ratio', domain=(0.08, 0.18)),
            dict(name='Nz: ultimate load factor', domain=(2.5, 6)),
            dict(name='Wdg: flight design gross weight', domain=(1700, 2500)),
            dict(name='Wp: paint weight', domain=(0.025, 0.08))]
...
def get_environmental_model():
    ...
    axes = [dict(name='M: mass of pollutant spilled at each location', domain=(7, 13)),
            dict(name='D: diffusion rate in the channel', domain=(0.02, 0.12)),
            dict(name='L: location of the second spill', domain=(0.01, 3)),
            dict(name='tau: time of the second spill', domain=(30.01, 30.295)),
            dict(name='s: space vector', domain=(0, 3)),
            dict(name='t: time vector', domain=(0.3, 60))]
...
def get_ebola_spread(country='Liberia'):
    ...
    def function(Xs):
        ...
        beta_1 = Xs[:, 0]  # Transmission rate between infected and susceptible
        beta_2 = Xs[:, 1]  # Transmission rate etween removed but still infectious and susceptible
        beta_3 = Xs[:, 2]  # Transmission rate between hospitalized and susceptible
        rho_1 = Xs[:, 3]  # Proportion of infected who die of the disease and are not hospitalized
        gamma_1 = Xs[:, 4]  # (Average time with disease for unhospitalized individuals)^{-1}
        gamma_2 = Xs[:, 5]  # (Average time with disease for hospitalized individuals)^{-1}
        omega = Xs[:, 6]  # Rate of proper burial
        psi = Xs[:, 7]  # Hospitalization rate

        R_0 = (beta_1 + (beta_2 * rho_1 * gamma_1)/omega + beta_3/gamma_2*psi) / (gamma_1 + psi)  # Reproduction number
...
def get_thermal_slab():
    """
    6 dimensional model arising from the solution of a heat equation on a 1D slab. Heat flux is injected into one
    end; the output is the temperature T(x, t) at each point of the slab and instant of time

    Reference:

    - T. Chantrasmi†, P. Constantine, N. Etemadi,G. Iaccarino and Q. Wang: "Uncertainty quantification in simple linear
    and non-linear problems" (2006). https://web.stanford.edu/group/ctr/ResBriefs06/1_UQ.pdf

             ** SENSITIVITY INDICES **
    Variable       Sobol     Total    Shapley    Banzhaf-Coleman
    ----------  --------  --------  ---------  -----------------
    q           0.008473  0.021518   0.014542           0.014316
    L           0.013357  0.023344   0.017937           0.017730
    k           0.002651  0.077777   0.036162           0.034136
    pCp         0.081799  0.133360   0.106213           0.105530
    xL          0.445545  0.603518   0.520085           0.517862
    t           0.243972  0.373525   0.305061           0.303217
    ...
    Dimension Metric              Value    Rel. Variance  Variables
    -------------------------  --------  ---------------  -------------
    Mean dimension             1.233042
    Effective (superposition)  2.000000         0.972923
    Effective (successive)     3.000000         0.960325
    Effective (truncation)     4.000000         0.955445  k, pCp, xL, t
    ...
    def function(Xs):
        ...
        sum = np.zeros(len(Xs))
        for n in range(1, 7):
            sum += 1./(n**2) * np.exp(-n**2*np.pi**2*(k/pCp*t)/L**2) * np.cos(n*np.pi*xL)

        return q*L/k * ((k/pCp)*t/(L**2) + 1./3 - xL + 1./2*xL**2) - (2*q*L)/(np.pi**2*k)*sum

    axes = [dict(name='q', domain=(3000, 4500)),
            dict(name='L', domain=(0.019, 0.019*1.5)),
            dict(name='k', domain=(0.0127, 0.1073)),
            dict(name='pCp', domain=(2.047*1e5, 6.063*1e5)),
            dict(name='x/L', domain=(0, 1)),
            dict(name='t', domain=(0, 1000))]

    return function, axes


def get_steel_column():
    """
    Stress of a steel column as a function of 8 parameters

    Reference:

    - Vicente Romero, Laura Swiler, Mohamed Ebeida and Scott Mitchell: "A Set of Test Problems and Results in Assessing
    Method Performance for Calculating Low Probabilities of Failure" (2016),
    http://www.cs.sandia.gov/~samitch/papers/POF-AIAA-paper.pdf

             ** SENSITIVITY INDICES **
    Variable       Sobol     Total    Shapley    Banzhaf-Coleman
    ----------  --------  --------  ---------  -----------------
    P1          0.022660  0.027558   0.025062           0.025038
    P2          0.073418  0.089289   0.081200           0.081123
    P3          0.073418  0.089289   0.081200           0.081123
    B           0.006917  0.008784   0.007825           0.007812
    T           0.238578  0.285026   0.261411           0.261216
    H           0.011637  0.025016   0.018108           0.017999
    D0          0.491118  0.560219   0.525194           0.524956
    ...
    Dimension Metric              Value    Rel. Variance  Variables
    -------------------------  --------  ---------------  -----------------
    Mean dimension             1.085181
    Effective (superposition)  2.000000         0.997111
    Effective (successive)     3.000000         0.967409
    Effective (truncation)     5.000000         0.966248  P1, P2, P3, T, D0
    ...
    def function(Xs):
        ...
        L = 7.5
        P1 = Xs[:, 0]  # Dead weight load, kN
        P2 = Xs[:, 1]  # Variable load, kN
        P3 = Xs[:, 2]  # Variable load, kN
        B = Xs[:, 3]  # Flange breadth, mm
        T = Xs[:, 4]  # Flange thickness, mm
        H = Xs[:, 5]  # Profile height, mm
        D0 = Xs[:, 6]  # Initial deflection, mm
        E = Xs[:, 7]  # Elastic modulus, MPa

        Eb = np.pi**2*E*B*T*H**2/(2*L**2)
        P = P1 + P2 + P3
        return P*(1/(2*B*T) + D0/(B*T*H)*Eb/(Eb-P))

    axes = [dict(name='P1', domain=(250, 650)),
            dict(name='P2', domain=(150, 870)),
            dict(name='P3', domain=(150, 870)),
            dict(name='B', domain=(185, 215)),
            dict(name='T', domain=(11.5, 27.5)),
            dict(name='H', domain=(75, 125)),
            dict(name='D0', domain=(-20, 80)),
            dict(name='E', domain=(1, 41))]

    return function, axes


def get_wind_turbine_wake():

    # reference: https://iopscience.iop.org/article/10.1088/1742-6596/625/1/012039/pdf
...
def get_track_bike():
    ...
    # From P. 189 in "Bicycling science"
    Cd = 0.77
    A = 0.35  # Frontal area
